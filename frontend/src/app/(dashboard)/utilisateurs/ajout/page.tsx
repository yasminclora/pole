...
import { UserPlus, Check, Loader2, ArrowLeft, RefreshCw } from 'lucide-react'
...
export default function AjoutUtilisateurPage() {
  ...
  const email = form.prenom && form.nom
    ? `${form.prenom.toLowerCase().trim().replace(/ /g,'')}.${form.nom.toLowerCase().trim().replace(/ /g,'')}@optima.dz`
    : ''
  ...
  const equipesFiltrees = equipes.filter(e => e.id_pole === Number(form.id_pole))
  ...
  if (!form.id_pole && !ROLES_SANS_POLE.includes(form.role)) erreurs.push('Pôle requis')
  if (!form.date_naissance)  erreurs.push('Date de naissance requise')
  if (!form.date_embauche)   erreurs.push("Date d'embauche requise")
  if (form.date_naissance && new Date(form.date_naissance).getFullYear() > 2006) erreurs.push('Doit avoir au moins 20 ans')
  if (form.date_embauche && new Date(form.date_embauche) >= new Date()) erreurs.push("Date d'embauche doit être dans le passé")
  if (form.telephone && errTel) erreurs.push('Téléphone invalide')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await usersService.creer({
        ...
        id_equipe      : (besoinEquipe && form.id_equipe) ? Number(form.id_equipe) : null,
      })
      setSucces(true)
    } catch (err: any) {
  ...
  if (succes) {
    return (
      <div className="max-w-md mx-auto mt-20 text-center">
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-5 mb-6 text-left space-y-3 border border-gray-200 dark:border-gray-700">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">
            Identifiants à communiquer à l'employé
          </p>
          <div className="flex justify-between items-center text-sm">
            <span className="text-gray-500">Email</span>
            <span className="font-mono text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded-lg">
              {email}
            </span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-gray-500">Mot de passe</span>
            <span className="font-mono text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded-lg">
              {motDePasse}
            </span>
          </div>
        </div>
        <div className="flex gap-3 justify-center">
          <button onClick={() => {
            setSucces(false)
            setForm({ nom:'',prenom:'',genre:'',date_naissance:'',date_embauche:'',telephone:'',role:'',id_pole:'',id_equipe:'' })
          }}
          ...
          <button onClick={() => router.push('/utilisateurs/liste')}
            className="px-5 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 font-medium transition-all">
            Voir la liste
          </button>
        </div>
      </div>
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {/* ── BANNIÈRE OPTIMA STANDARDISÉE (Pleine Largeur) ── */}
      <div className="relative overflow-hidden w-full bg-[#1e3a67] text-white rounded-2xl p-6 shadow-sm border border-slate-800/20 bg-[radial-gradient(#ffffff10_1px,transparent_1px)] [background-size:16px_16px]">
        <div className="flex items-center justify-between relative z-10">
          ...
          <button 
            ...
            className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition-all text-white/80 hover:text-white"
      ...
      {/* ── FORMULAIRE COMPLET (Pleine Largeur) ── */}
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                Téléphone
                <span className="text-gray-400 font-normal ml-1 text-xs">(05/06/07 + 8 chiffres)</span>
              </label>
              <input
                ...
                maxLength={10}
                className={`${inputClass} ${
                  errTel ? 'border-red-400 focus:ring-red-400' : form.telephone && !errTel ? 'border-green-400 focus:ring-green-400' : ''
                }`}/>
            ...
            <div>
              ...
              <input type="date" value={form.date_naissance}
                ...
                className={inputClass}/>
              {form.date_naissance && new Date(form.date_naissance).getFullYear() > 2006 && (
                <p className="text-red-500 text-xs mt-1">⚠ Doit avoir au moins 20 ans</p>
              )}
            </div>
            ...
            <div>
              ...
              <input type="date" value={form.date_embauche}
                ...
                className={inputClass}/>
              {form.date_embauche && new Date(form.date_embauche) >= new Date() && (
                <p className="text-red-500 text-xs mt-1">⚠ Doit être antérieure à aujourd'hui</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {!ROLES_SANS_POLE.includes(form.role) ? (
              ...
              <div>
                ...
                <div className="px-3 py-2.5 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-500 text-sm border border-gray-200 dark:border-gray-700">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                  ...
                  {form.role === 'CHEF_EQUIPE' && (
                    <span className="text-amber-500 font-normal ml-2 text-xs">(uniquement les équipes sans chef)</span>
                  )}
                </label>
                <select value={form.id_equipe}
                  ...
                  {equipesDisponibles.map(eq => (
                    <option key={eq.id_equipe} value={eq.id_equipe}>{eq.nom_equipe}</option>
                  ))}
                </select>

                {form.id_pole && equipesFiltrees.length === 0 && (
                  <p className="text-amber-500 text-xs mt-1">⚠ Aucune équipe dans ce pôle</p>
                )}
                {form.role === 'CHEF_EQUIPE' && form.id_pole && equipesFiltrees.length > 0 && equipesDisponibles.length === 0 && (
                  <p className="text-red-500 text-xs mt-1">⚠ Toutes les équipes de ce pôle ont déjà un chef</p>
                )}
                {form.role === 'CHEF_EQUIPE' && form.id_pole && equipesFiltrees.length !== equipesDisponibles.length && (
        ...
        {(email || motDePasse) && (
          <div className="bg-gray-50 dark:bg-gray-800/50 border border-dashed border-gray-300 dark:border-gray-700 rounded-2xl p-5">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">
              Identifiants générés automatiquement
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {email && (
                <div className="flex items-center justify-between bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800/50 p-3 rounded-xl">
                  <span className="text-sm text-gray-500 dark:text-gray-400">Email</span>
                  <span className="font-mono text-sm text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-3 py-1 rounded-lg">
                    {email}
                  </span>
                </div>
              )}
              {motDePasse && (
                <div className="flex items-center justify-between bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800/50 p-3 rounded-xl">
                  <span className="text-sm text-gray-500 dark:text-gray-400">Mot de passe initial</span>
                  <span className="font-mono text-sm text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-3 py-1 rounded-lg">
                    {motDePasse}
                  </span>
                </div>
              )}
        ...
        <div className="flex gap-3 justify-end pb-6">
          <button type="button"
            onClick={() => router.push('/utilisateurs/liste')}
            className="px-6 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-all">
            Annuler
          </button>
          <button type="submit"
            disabled={loading || !formulaireValide}
            className="flex items-center gap-2 px-8 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium disabled:opacity-40 disabled:cursor-not-allowed transition-all">
            {loading
